...
export default function WeeklyMasses() {
  ...
  const fetchMasses = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/admin/weekly-masses`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId 
        ? `${apiUrl}/api/admin/weekly-masses/${editingId}`
        : `${apiUrl}/api/admin/weekly-masses`
      ...
      if (data.success) {
        ...
        alert(data.error || 'Failed to save weekly mass')
      }
    } catch (error) {
      alert('Error saving weekly mass')
    }
  }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('adminToken')
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'
      const response = await fetch(`${apiUrl}/api/admin/weekly-masses/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      alert('Error deleting weekly mass')
    }
  }
